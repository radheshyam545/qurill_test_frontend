...
const NotificationEmployer = () => {
    ...
    const getNotification = async () => {
        ...
        // fetch notification data
        try {
            const response = await getCall('/employers/notifications', { limit: PER_PAGE, page: (currentPage + 1) })
                    ...
                    {notificationData?.length > 0 && notificationData.map((item) => {
                        return (
                            <div className="cursor-pointer notification-card flex items-center gap-[15px] rounded-[15px] p-[10px] border border-[#d8d8d8] mb-8"
                                onClick={() => { navigate(`/employer/Job-Detail/${item?.jobId}`)}}
                            >
                                <div className="min-w-[35px] [w-[35px] h-[35px] rounded-full -bg-[#E5EBFE] bg-cover bg-no-repeat bg-center" style={{backgroundImage: `url(${item?.seekerPhoto ? item?.seekerPhoto : '/assets/images/notification.png'})`,}} ></div>
                                <div className="min-w-[150px] max-w-[250px]">
                                    <h5 className="font-bold text-[15px] mb-[2px]">{item?.seekerName || ""}</h5>
