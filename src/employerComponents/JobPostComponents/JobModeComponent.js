import React from 'react';

function JobModeComponent({ tab, setTab, postJobData, setPostJobData }) {
  const handleJobTypeClick = (mode) => {
    // If the mode is already selected, unselect it
    if (postJobData?.mode === mode) {
      setPostJobData(prevData => ({
        ...prevData,
        mode: ""
      }));
    } else {
      // Otherwise, select the new mode
      setPostJobData(prevData => ({
        ...prevData,
        mode: mode
      }));
    }
  };

  const closeTab = () => {
    setTab('0');
  };

  return (
    <div className={`h-[100vh] z-[999] flex items-center justify-center w-[100%] fixed top-0 left-0 overlay-color`}>
      <div className='h-[300px] w-[700px] bg-white flex flex-col justify-between'>
        <div className='flex items-center justify-between h-[40px] px-[30px] py-[30px] border-b'>
          <p className='font-bold'>Edit the job post</p>
          <img onClick={closeTab} className='cursor-pointer h-[15px]' src="/assets/images/employer-images/jobpostoverlaytabcross.svg" alt="" />
        </div>
        <div className='px-[20px]'>
          <p className='font-bold'>Job type</p>
          <div className='flex gap-[10px] flex-wrap mt-[5px] mb-[50px]'>
            <button
              onClick={() => handleJobTypeClick('remote')}
              className={` ${postJobData.mode === 'remote' ? 'jobtypeActive' : ''} px-[15px] py-[5px] rounded-[18px] font-semibold bg-[#F2F2F2] border border-[#B3B3B3] text-[18px]`}
            >
              Remote
            </button>
            <button
              onClick={() => handleJobTypeClick('on_Site')}
              className={` ${postJobData.mode === 'on_Site' ? 'jobtypeActive' : ''} px-[15px] py-[5px] rounded-[18px] font-semibold bg-[#F2F2F2] border border-[#B3B3B3] text-[18px]`}
            >
              On Site
            </button>
            <button
              onClick={() => handleJobTypeClick('hybrid')}
              className={` ${postJobData.mode === 'hybrid' ? 'jobtypeActive' : ''} px-[15px] py-[5px] rounded-[18px] font-semibold bg-[#F2F2F2] border border-[#B3B3B3] text-[18px]`}
            >
              Hybrid
            </button>
          </div>
        </div>
        <div className='h-[60px] w-[100%] border-t px-[30px] py-[10px]'>
          <button className='px-[10px] py-[5px] rounded-[5px] bg-[#FFCB05] font-bold float-right' onClick={closeTab}>Done</button>
        </div>
      </div>
    </div>
  );
}

export default JobModeComponent;
